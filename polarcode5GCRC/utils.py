...
class BTreeNode:
    def __init__(self, idx, leaf=False):
        self.idx = idx
        self.parent = None
        self.left = None
        self.right = None
...
def build_binary_tree(N, depth):
    # Decoding structure construction (build a complete binary tree)
    total_nodes = int(2 ** (depth + 1) - 1)

    leaf_idx = total_nodes - N
    decoding_tree = np.array([BTreeNode(i) for i in range(leaf_idx)] +
                             [BTreeNode(i, leaf=True) for i in range(leaf_idx, total_nodes)])

    decoding_tree[0].length = N
    decoding_tree[0].end = N

    # Label child nodes
    for i in range(total_nodes - N):
        decoding_tree[i].left = decoding_tree[2 * decoding_tree[i].idx + 1]
        decoding_tree[i].right = decoding_tree[2 * decoding_tree[i].idx + 2]

    # Label parents
    for i in range(1, total_nodes):
        parent_idx = (decoding_tree[i].idx - 1 - (i % 2 == 0)) // 2
        decoding_tree[i].parent = decoding_tree[parent_idx]
        decoding_tree[i].depth = decoding_tree[parent_idx].depth + 1
        decoding_tree[i].length = decoding_tree[parent_idx].length // 2

        if decoding_tree[parent_idx].left.idx == decoding_tree[i].idx:
            decoding_tree[i].start = decoding_tree[i].parent.start
            decoding_tree[i].end = decoding_tree[i].start + decoding_tree[i].length
        else:
            decoding_tree[i].end = decoding_tree[i].parent.end
            decoding_tree[i].start = decoding_tree[i].end - decoding_tree[i].length

        # # label leaf nodes
        # if i >= leaf_idx:
        #     decoding_tree[i].leaf_node_idx = i - N + 1
...
def full_reliability_sequence_5g():
    return np.array([
        0,     1,     2,     4,     8,    16,    32,     3,
...
if __name__ == '__main__':
    N = 8
    d = log2(N)
    tree = build_binary_tree(N, d)
    for t in tree:
        print(t, ":", t.left, t.right, t.depth, t.start, t.end, t.length)
